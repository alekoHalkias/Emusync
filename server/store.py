...
class Store:
    ...
    def add_game(self, game: Game) -> Game:
        ...
        if existing:
            # Update existing game without cascade-deleting saves/locks
            self._conn.execute(
    ...
    # ── game_devices ──────────────────────────────────────────────────────────
    # Note: game_devices table is kept for backwards compatibility but is no longer actively used
    # All game config is now stored in the games table
    ...
    def set_game_device(self, gd: GameDevice) -> None:
        self._conn.execute(
            """INSERT OR REPLACE INTO game_devices
               (game_slug, device_id)
               VALUES (?, ?)""",
            (gd.game_slug, gd.device_id),
        )
        self._conn.commit()

    def get_game_device(self, game_slug: str, device_id: str) -> Optional[GameDevice]:
        row = self._conn.execute(
            """SELECT game_slug, device_id FROM game_devices WHERE game_slug = ? AND device_id = ?""",
            (game_slug, device_id),
        ).fetchone()
        if not row:
            return None
    ...
    def push_save(self, game: str, device_id: str, data: bytes) -> SaveMeta:
        ...
        self._conn.commit()
        return SaveMeta(game_slug=game, device_id=device_id, hash=h, pushed_at=now)
    ...
    def get_lock(self, game: str) -> Optional[Lock]:
        row = self._conn.execute(
            "SELECT game, device_id, acquired_at FROM locks WHERE game = ?",
            (game,),
        ).fetchone()
        if not row:
            return None
        return Lock(game_slug=row["game"], device_id=row["device_id"], acquired_at=row["acquired_at"])

...
class Store:
    def __init__(self, data_dir: str) -> None:
        ...
        try:
            ...
            pass  # column already exists
        # Migration: add console column to game_devices (safe to run multiple times)
        try:
            self._conn.execute(
                "ALTER TABLE game_devices ADD COLUMN console TEXT NOT NULL DEFAULT ''"
            )
        except sqlite3.OperationalError:
            pass  # column already exists
        self._conn.commit()
    ...
    def set_game_device(self, gd: GameDevice) -> None:
        self._conn.execute(
            """INSERT OR REPLACE INTO game_devices
               (game_slug, device_id, rom_path, save_path, launch_command, state_path, rom_folder_path, console)
               VALUES (?, ?, ?, ?, ?, ?, ?, ?)""",
            (gd.game_slug, gd.device_id, gd.rom_path, gd.save_path, gd.launch_command, gd.state_path, gd.rom_folder_path, gd.console),
        )
        self._conn.commit()

    def get_game_device(self, game_slug: str, device_id: str) -> Optional[GameDevice]:
        row = self._conn.execute(
            """SELECT game_slug, device_id, rom_path, save_path, launch_command, state_path, rom_folder_path, console

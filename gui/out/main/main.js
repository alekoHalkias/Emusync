...
function startSyncDaemon() {
  ...
  try {
    syncDaemonProcess = child_process.spawn(PYTHON, [SCRIPT, "sync-daemon"], {
      stdio: "ignore",
      detached: true,
      env: { ...process.env }
    });
...
electron.ipcMain.handle("game:pushToServer", (_event, slug) => {
  const proc = child_process.spawn(PYTHON, [SCRIPT, "push", slug], {
    stdio: "ignore",
    detached: true,
    env: { ...process.env }
  });
...
electron.app.on("window-all-closed", () => {
  if (syncDaemonProcess) {
    try {
      process.kill(-syncDaemonProcess.pid, "SIGTERM");
    } catch {
      syncDaemonProcess?.kill("SIGTERM");
    }

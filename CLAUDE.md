...
| `gui/renderer/src/App.tsx` | Root component; screen router; auto-starts server if `is_server=true`; on the console screen the topbar shows the console abbr/label/game-count (reusing `GameGrid`'s `.game-grid-abbr`/`-label`/`-total` classes) next to the "‹ Back" link — grouped in one flex item since `.topbar` uses `justify-content: space-between` (issue #349); this "‹ Back" link (issue #351) is the only back control now — `GameGrid` dropped its own redundant "‹ Back" button (issue #350); the mouse's back side button (`MouseEvent.button === 3`) also triggers it, via a `mouseup` listener registered only while on the console screen (issue #354) |

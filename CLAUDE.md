...
| `server/api.py` | FastAPI routes; auth via `Authorization: Bearer {PIN}` + `X-Device-ID`/`X-Device-Name` headers; `/health`, `/games`, `/devices`, `/whoami`, `/saves`, `/states`, `/roms`, `/file` (serve arbitrary files by path for device-to-device transfer), `/locks`, `/events`, `/games/{slug}/devices`, `/consoles` (GET/PUT); `_auth` auto-registers devices on first request and calls `touch_device()` to record client IP + timestamp |

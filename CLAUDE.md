...
**DB schema versioning — use `PRAGMA user_version`, not try/except** — `store.py` tracks the schema version in `PRAGMA user_version` (currently `_SCHEMA_VERSION = 6`). When adding a new migration: (1) add a new `if from_version < N:` block in `_migrate()`, (2) bump `_SCHEMA_VERSION` to N, (3) add the new table/column definitions to `_SCHEMA` so fresh DBs get them without running migrations. Do not add bare `try/except ALTER TABLE` blocks outside `_migrate()` — warm-start DBs skip `_migrate()` entirely via the version check.

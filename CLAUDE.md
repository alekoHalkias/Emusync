...
**A device's *first-ever* launch of a learned-save-path game (Switch, and Wii) used to always play blind, discarding any progress already synced from another device (#443).** `save_path` stays `""` until a session's write is unambiguously resolved, and pre-launch reconciliation has nowhere to pull *to* without a known destination — so the first session on a new device started a fresh save, and the post-launch adoption step then pushed that fresh save over the server's real progress (auto-resolved newest-wins, which could pick the wrong side). Fixed for **Switch only**: `cli/run_switch.py`'s `_seed_switch_save` pre-seeds the server's existing save into *every* existing Eden profile folder before launch, keyed by the title ID parsed from the ROM's bracketed filename tag (`_switch_title_id_from_rom`, reusing `cli.detect._SWITCH_TITLE_ID_RE` — the same convention that already filters base/update/DLC files, #419). Seeded files get an mtime before `launch_start` is recorded, so `_resolve_written_switch_save`'s existing "touched since launch" detection still correctly identifies exactly one folder afterward even with multiple profiles seeded. **Wii is deliberately excluded** — there's no established filename convention for Wii's Game ID (unlike Switch's scene-standard bracket tagging), and guessing wrong would risk seeding a save into the wrong title's NAND folder. A Switch ROM without the bracketed title-ID tag falls back to the prior blind-first-play behavior — not a regression, just no improvement for that file.

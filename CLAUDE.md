...
| `gui/electron/art.ts` | `art:get` IPC handler: checks `~/.emusync/art/<slug>.png` cache, then downloads from `raw.githubusercontent.com/libretro-thumbnails/<System>/master/Named_Boxarts/<GameName>.png`; atomic temp→rename write; returns `file://` URL or null (issue #304) |

...
| `server/sync_client.py` | HTTP client wrapping all server endpoints (used by `emusync run`, `pull`, `push`); sends PIN + device headers for auth; `GameDeviceConfig` holds `rom_path`, `save_path`, `launch_command`, `state_path`, `rom_folder_path` — all 5 are sent to `PUT /games/{slug}/device`; `pull_rom()` and `push_rom()` support streaming with progress callbacks |

...
| `gui/renderer/src/components/GameGrid.tsx` | Per-console game grid: `GameCard` tiles split into "On this device"/"On other devices"; live search filter, `GameFilterButton` popover (#345), artwork-type dropdown (grid/wide_grid/hero/logo/icon, persisted to `art_type_by_console` — #324, wide_grid #333; changing it remounts every visible `GameCard` via a type-inclusive `key` so they re-fetch); no back button of its own (dropped as redundant, #350/#351) or `consoleLabel`/`consoleAbbr` props (moved to `App.tsx`'s topbar, #349); opens `GameModal`/`NetworkPlaySetup` as needed (#304); multi-select bulk delete offers the same tiered `deleteGame` checkboxes as `GameConfig.tsx` (#343); a Select All/Deselect All button (`toggleSelectAll`) sits left of Delete, toggling over `filtered` (the currently visible games after search + filter, not the full list — #353); between them sit two bulk buttons sharing one dismissible summary line (`bulkMsg`): "Download" (#396) localizes every selected network-sourced game via `rom:localize`, driving a byte-level `DownloadProgressModal` ("Downloading game n of N" + current name + overall bar + bytes/speed + Cancel): batch total from `rom:localizeSizes`, per-chunk progress via `rom:onLocalizeProgress`, speed as an EMA over ≥500ms windows, Cancel via `rom:cancelLocalize` (current game's `.part` cleaned up, summary "cancelled — N of M completed"); silently skips local-source/already-localized games — a console with no local folder configured prompts the folder picker once on its first game (`rom:localize` persists the choice, so the rest of the loop doesn't re-prompt) — then calls `onChanged()` so 💾 badges and the Localized filter update; and "Add to Steam" (#391): adds every selected game via `steam:addGame`, silently skipping slugs already in the `steamSlugs` set (loaded once per mount via the batched `steam:addedSlugs`, refreshed after each bulk run so the Steam filter updates in place); both abort on the first error (e.g. Steam running) surfacing it once ("Added N, skipped M already added"); all header buttons always render (disabled when inapplicable) rather than appear/disappear, sized to match the dropdown/search box via `.game-grid-header-btn` (#347) |
| `gui/renderer/src/components/GameFilterButton.tsx` | Filter popover next to the search box (#345): 4 checkbox groups — Artwork (with/without), Saves (on device/not), ROM availability (localized/not), Steam (in/not in, #391) — OR-within-group, AND-across-groups via exported `matchesFilters`. "Localized" checks whether the ROM's bytes are actually on this device right now (`romSource !== "network" \|\| hasLocalCopy`), not just whether the game is network-sourced. Saves reads straight off `GameRow.lastSave`; artwork has no such field, so `GameGrid` lifts each `GameCard`'s `art:get` result via `onArtStatus` into a `hasArt` map instead of a duplicate fetch — an unresolved game (`undefined`) passes the filter rather than being hidden while loading; Steam status likewise passes as `undefined` until `GameGrid`'s one batched `steam:addedSlugs` call resolves. Closes on outside click; badges the button with the active filter count |
...
| `gui/renderer/src/components/SteamRestartModal.tsx` | "Steam is open — restart it?" yes/no confirm (#393), shared by `GameConfig`'s single add and `GameGrid`'s bulk add. Purely presentational (`count`/`busy`/`onYes`/`onNo`); each caller owns the `steam:shutdown` → add → `steam:launch` orchestration |
| `gui/renderer/src/components/DownloadProgressModal.tsx` | Byte-level progress modal for the bulk ROM download (#396): "Downloading game n of N", current game name, overall progress bar (`.download-progress-track`/`-fill`), bytes done / batch total + speed, Cancel bottom-right. Purely presentational (`DownloadModalState` + `cancelling`/`onCancel`); `GameGrid` owns the loop, progress subscription, and cancellation |
...
window.emusync.rom.localizeSizes(slugs)                  // stat each slug's network master, {slug: bytes} (0 if unreachable) — batch total for the download modal's overall bar (#396)
window.emusync.rom.cancelLocalize()                      // cancel the in-flight localize; its .part is cleaned up and it returns {ok:false, cancelled:true} (#396)
window.emusync.rom.onLocalizeProgress(cb)                // subscribe to rom:localize-progress ({slug, copied, total}, throttled ~4MB) emitted by the streamed localize copy; returns the listener for offLocalizeProgress (#396)
...
window.emusync.steamgriddb.setKey(key)     // PUTs a new shared key; editable from any device's settings panel so a headless server can be configured (#322, #398)
window.emusync.steamgriddb.openKeyPage()   // opens steamgriddb.com/profile/preferences/api

window.emusync.steam.addGame(slug, gameName, consoleName, consoleKey) // adds/updates a Steam non-Steam-game shortcut + artwork + best-effort console Collection; {ok, updated?, warning?, error?} (#385)
window.emusync.steam.isAdded(slug)         // read-only: whether this game already has an EmuSync Steam shortcut (safe with Steam running; false on any failure so the Add button stays enabled — #389)
window.emusync.steam.addedSlugs()          // read-only batch form: every slug with an EmuSync Steam shortcut, one shortcuts.vdf parse for the whole grid ([] on failure) — backs the Steam filter + bulk add (#391)
window.emusync.steam.isRunning()           // Linux pid-file liveness (false on Windows); gates the restart-to-add dialog (#393)
window.emusync.steam.shutdown()            // graceful `steam -shutdown` (NOT a pid kill — Steam rewrites shortcuts.vdf/collections on exit), polls up to 30s until exited + 2s flush grace; {ok, error?} (#393)
window.emusync.steam.launch()              // detached Steam relaunch after the writes; failure is a warning, the shortcuts were already added; {ok, error?} (#393)
...
**Shared-save-layout consoles (PS2/DC/GC/PSP) — never treat the save as per-game** — For consoles where the save lives in ONE shared location across every game, the per-game save must not be renamed, moved, or pushed per-game; it syncs as a console-scoped card via `/consoles/{key}/memcard` + `emusync run`'s `_MemcardClient`. Card paths (resolved at import by `scan.ts`'s `resolveSharedCard` / `detect.py`'s `_resolve_shared_memcard_save_state`, first existing candidate wins): PS2 `memcards/Mcd001.ps2` (#294/#295); Dreamcast `saves/vmu_save_A1.bin` (VMU slot A1 only); GameCube/Wii Dolphin's `User/GC` or `system/dolphin-emu/Userdata/GC` folder (Wii NAND title saves NOT synced); PSP `PPSSPP/PSP/SAVEDATA` folder, all games one blob (#402). **Save STATES are a separate axis**: only PS2 shares states (serial-named `sstates/`, #294) — dc/gamecube/psp cores write normal per-content RetroArch states with full per-game state sync. Renderer gates: `usesSharedSaveLayout` (all four — import passes empty savePath to `renameGameFiles`, per-game save autoPush/pull skipped, GameConfig hides the per-game save `SyncLine` and shows the console-scoped memcard push/pull `SyncLine` #319) vs `usesSharedStateLayout` (`{ps2}` only — same treatment for state paths; a per-game PS2 state pull would `.bak` every game's slots) in `console-import/helpers.ts`. CLI mirror: `run_ps2.py`'s `_SHARED_MEMCARD_CONSOLES` (`{PS2,DC,GC,PSP}`, keyed by abbr)/`_SHARED_STATE_CONSOLES` (`{PS2}`) and `scan.ts`'s `SHARED_MEMCARD_CONSOLES`/`SHARED_STATE_CONSOLES`. Keep all six in sync if adding a shared-layout console. **Import-time pull (#316):** `useConsoleImport`'s `pullFromServerIfNewer` pulls the console-scoped card via `window.emusync.memcard.pull` right after import if the server's copy is newer (or local has none) — the pull counterpart to `autoPush`, console-keyed. PS2's shared save *states* are deliberately NOT pulled at import time — keyed by game serial with no console-scoped endpoint, so only `emusync run`'s per-launch serial-filtered sync touches them; dc/gamecube/psp states pull per-game like any other console.

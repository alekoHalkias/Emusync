...
**DB schema initialization — do not use deprecated `executescript()`** — `store.py` initializes tables by splitting `_SCHEMA` and executing each statement individually rather than calling `executescript()`, which is deprecated and incompatible with WAL mode and complex SQL syntax like `ON CONFLICT`. Device registration uses try/except for INSERT vs UPDATE instead of `ON CONFLICT` clauses for broader compatibility.

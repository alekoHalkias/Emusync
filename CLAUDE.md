...
**Switch update/DLC files are auto-detected AND manually managed — check both before assuming a file is missing (#441).** `game_devices.update_paths` (`;`-joined, `GameDeviceConfig.update_paths: list[str]` at the API boundary) holds files the console-import wizard found next to the base ROM at import time (`cli/detect.py`'s `_find_switch_update_files`, matched by shared 13-hex title-ID prefix); the separate `~/.emusync/switch_updates/<slug>/` managed folder holds files explicitly added via `emusync update add`/the GUI's Updates tab. `cli/update.py`'s `_resolve_update_files` (and `gui/electron/sync/update.ts`'s `resolveUpdateFiles`) merge both into one view for `list`/`push` — don't read `update_paths` or the managed folder alone and assume that's everything available.

...
| `cli/` | Click CLI implementation, one module per command group. `cli/root.py` defines the root `cli` group; `cli/__init__.py` imports every command module (registering subcommands) and re-exports `cli`. `cli/common.py` = shared helpers (`_client`, `_print_table`, `_get_device_name`, `_show_game_running_popup`, and `_fmt_time`/`_relative`/`_parse_iso_utc` for human-readable timestamps — issue #216); `cli/consoles_data.py` = hardcoded `_IMPORT_CONSOLES`/`_IMPORT_SYSTEMS`/extension sets + `_prepare_console_seed_data` (seeds the server's global console defs); `cli/detect.py` = RetroArch/core/ROM detection + scan helpers (mirrors `main.ts`); `cli/server.py` = `server` group (start/stop/restart/clear-devices/discover-json) + lifecycle helpers + embedded transfer daemon; `cli/device.py` = `device` group (`connect`/`list`/`compare`, where `compare` shows game coverage across paired devices); `cli/game.py` = `game` group (`add`/`list`/`edit`/`remove`); `cli/console.py` = `console` group (`list` + `import`, the interactive wizard mirroring the GUI Add Console flow: detect RetroArch/cores/standalones → scan ROM folder → bulk import); `cli/sync.py` = `sync status` + `sync history <slug>` / `sync restore <slug> <version-id>` (save/state rollback, issue #7 — `--state` flag targets states; restore makes the chosen version current on the server and writes it to this device's disk if configured); `cli/transfer.py` = top-level `push`/`pull`/`sync-daemon` + the transfer daemon loop (`push` streams a ROM to a target device; `pull` requests a ROM from a source device via a pull request the source's sync-daemon fulfills; `sync-daemon` holds an SSE connection open, auto-receives incoming transfers, and fulfills pending pull requests; when `watch_saves` is set in config it also starts a background save watcher — see `cli/watch.py`); `cli/watch.py` = the opt-in background save/state watcher (issue #242): `run_save_watcher`/`SaveWatcher` poll each local game's `save_path`/`state_path` and push on change so saves written by a non-EmuSync-wrapped session (plain RetroArch-direct, #210) still sync. Dependency-free polling; a file must be *settled* (`SETTLE_SECONDS`) before pushing; saves reuse `_save_is_safe_to_push` (truncation guard); a game locked by another device is skipped. Wired in via `_run_transfer_daemon(..., watch_cfg=cfg)` from both `sync-daemon` and the server-embedded daemon; `cli/run.py` = `run` command + the SIGTERM handler that kills the emulator child (registered at import time). Before launch it reconciles the save via `_reconcile_save`/`_decide_save_action` (newest-wins: push if local mtime > server `pushed_at`, pull otherwise, loser kept as `.bak`) — for a shared-memory-card console (`_SHARED_MEMCARD_CONSOLES`, currently `PS2`) the same reconcile runs against the console card instead, via the `_MemcardClient` adapter that maps the per-game save API onto the `/consoles/{key}/memcard` endpoints; the post-launch written-path adoption is skipped for those consoles (issue #295). PCSX2 save states live in one SHARED folder named per game serial (`sstates/<SERIAL> (<CRC>).<slot>.p2s`), so for `_SHARED_STATE_CONSOLES` (`PS2`) states sync filtered by serial (issue #294): pre-launch `client.pull_state_merge` extracts only the archive's files (backing up just what it overwrites, leaving other games' states alone), and post-launch `_ps2_state_serial_prefix` derives the serial from the `.p2s` written this session and pushes only that serial's files via `push_state(..., name_prefix=...)`; a true divergence is surfaced via `_warn_save_conflict` (stderr + `_notify` desktop notification + `_log_save_conflict` → `save_conflicts.json` + `_report_conflict_to_server` → `POST /games/{slug}/conflicts` for the GUI Conflicts panel, issue #243). After the game exits, the post-game save push is gated by `_save_is_safe_to_push` (issue #213): a 0-byte save, or one that shrank below 50% of the server's previous copy (a truncation signal from a crashed emulator), is **not** pushed — the good server copy is kept and the refusal is surfaced via `_warn_unsafe_save` (stderr + notification). If `client.health()` fails it falls back to `_run_offline` — launches anyway and logs the play to `offline_plays.json`; `_cache_game_device`/`_load_cached_game_device` persist each game's paths under `game_cache/` so offline launches know the save path (the authoritative config lives on the server). For network-sourced ROMs (issue #255) `_resolve_launch_command` picks which copy to launch from — the live NAS master, else a localized copy (rewriting the command's ROM path), else refuse; `cli/netrom.py` = the network-ROM helpers (`path_is_reachable` bounded mount-liveness probe used before any network-path touch so a dead mount never hangs launch/watcher/push, rel-path `normalize`/`sanitize`/`compute_rel_path`/`join_network` for portable POSIX rel-paths over POSIX/UNC roots, and `localize_rom`/`delocalize_rom` for the free-space-checked, atomic-via-temp+rename, master-guarded copy, plus `upload_to_master` — the reverse of `localize_rom` (copy a local-only ROM UP to the share so it becomes the master; skip-if-exists so a present master is never overwritten — issue #270); `cli/rom.py` = `rom` group (`list`/`localize`/`delocalize` — copy a network ROM, or a whole `--console`, onto local disk for offline play and back; never touches the NAS master) |
| `server/api/` | FastAPI app, split into per-domain `APIRouter` modules (issue #224): `_core.py` owns the `app`, shared mutable state (`_store`/`_master_pin`/`_data_dir`/presence sets/`_device_event_queues`), `init()`, `_auth`, the activity-log helpers (`_print_activity`/`_device_label`/`_game_label`), ROM-staging helpers, and `/health`; `devices.py`/`games.py`/`transfers.py`/`blobs.py`/`locks.py`/`defs.py`/`conflicts.py` are the routers (`conflicts.py` = the save-conflict records behind the GUI Conflicts panel — `POST /games/{slug}/conflicts`, `GET /conflicts`, `POST /conflicts/{id}/dismiss`, issue #243); `__init__.py` re-exports `app`+`init` and `include_router`s them (so `from server import api` still gives `api.app`/`api.init`). Routers reach reassigned globals via accessors (`_get_store()`, `get_data_dir()`) and the live presence objects, never by re-importing the rebindable names. Auth via `Authorization: Bearer {PIN}` + `X-Device-ID`/`X-Device-Name` headers; `/health`, `/games`, `/games/overview` (per-device batch: every game's lock + last save + this device's config in one call — registered before `/games/{slug}` so it isn't matched as a slug; used by the GUI game list + lock poll instead of fanning out 3 requests per game), `/devices`, `/whoami`, `/saves`, `/states`, `/locks`, `/events`, `/events/stream` (SSE), `/games/{slug}/devices`, `/games/{slug}/network-source` (any device's network-share config for a game so a device without it can join the rel-path to its own mount root — issue #270), `/game-devices`, `/devices/{id}/consoles`, `/devices/{id}/game-devices`, `/games/{slug}/rom-transfer`, `/rom-transfers/pending`, `/rom-transfers/{id}/file`, `/rom-transfers/{id}`, `/games/{slug}/rom-pull-request`, `/rom-pull-requests/pending`, `/rom-pull-requests/{id}`, `/console-defs`, `/system-defs`, `/console-folder-names`, `/standalones/{console_key}`; save/state history + rollback (issue #7): `GET /games/{slug}/save/history` + `POST /games/{slug}/save/restore` (and the `/state/` equivalents), with `save/meta`/`state/meta` now also reporting `size`; in `blobs.py` the save and state route families are thin wrappers over shared `_BlobKind`-parametrised handlers (`_pull`/`_push`/`_meta`/`_history`/`_restore`) that dispatch on the noun to the uniform store methods (`pull_<noun>_path`, `push_<noun>_file`, …) — issue #240; save/state integrity (issue #285): `GET /games/{slug}/integrity` (recomputes a single game's save+state verdict on demand — 0-byte/shrank/hash-mismatch/missing), top-level `GET /integrity` (library-wide damaged summary from the at-rest snapshot) + `POST /integrity/rescan` (re-runs the sweep); the snapshot is built in `_core._run_integrity_sweep()` on startup (in `init()`, after `_sweep_stale_staging`) and read via `get_integrity_status()` — no schema column, computed from existing row metadata + on-disk bytes; console-scoped shared memory card (issue #295, in `blobs.py`): `GET/POST /consoles/{console_key}/memcard` + `GET /consoles/{console_key}/memcard/meta` — one card per console keyed by abbr (e.g. `PS2`), shared across all the console's games (mirrors the save routes but single-generation/console-keyed; used by PS2 via `run.py`'s `_MemcardClient`); `_auth` auto-registers devices on first request; `GET /devices` includes `is_online`; `init()` accepts optional `data_dir` for ROM staging; `_device_event_queues` maps device IDs to asyncio queues for SSE delivery; `settings.py` = the shared SteamGridDB API key (`GET`/`PUT /settings/steamgriddb-key`), a generic `server_settings` key-value store — entered once on the server device, fetched by every connected device's Electron process so art fetching doesn't need a per-user key (issue #322); `games.py`'s `GameRequest`/`PUT /games/{slug}` also accepts an optional `sgdb_game_id` — a manually-picked SteamGridDB match remembered per game (issue #325), left untouched when omitted (same truthy-check pattern as `console`); `DELETE /games/{slug}/device` (issue #343) unlinks the game from the calling device only — idempotent, the game/saves/other devices are untouched — vs. the existing `DELETE /games/{slug}` which fully purges the game everywhere |
| `server/store/` | SQLite via stdlib `sqlite3`, split into a package: `__init__.py` composes `Store` from one mixin per table-group and re-exports the public surface (`Store`, `LOCK_TTL_HOURS`, `upsert_console_for_game`, dataclasses); `connection.py` = `_ThreadLocalConnection` (one SQLite connection per thread); `schema.py` = `_SCHEMA`/`_SCHEMA_VERSION`/`_migrate`; `models.py` = row dataclasses; `_base.py` = `StoreBase` (connection + schema/migration setup); `devices.py`/`games.py`/`blobs.py`/`locks.py`/`events.py`/`transfers.py`/`consoles.py`/`console_defs.py` = the per-domain CRUD mixins. Tables: `devices`, `consoles`, `games`, `game_devices`, `saves`, `states`, `locks`, `events`, `rom_transfers`, `rom_pull_requests`, `console_defs`, `system_defs`, `core_defs`, `console_folder_names`, `standalone_emulators`, `save_conflicts`, `console_saves`, `server_settings`; schema version 15 (v15 adds `games.sgdb_game_id` — a manually-picked SteamGridDB game match remembered per game, shared across every device since it lives on the `games` row itself rather than per-device config; set via `PUT /games/{slug}`'s optional `sgdb_game_id` field, left untouched by a plain rename — issue #325); (v14 adds `server_settings` — a generic key-value table for server-wide settings, currently just the shared SteamGridDB API key entered once on the server device and fetched by every connected device, issue #322); (v13 adds `console_saves` — a console-scoped shared save: one memory card per `console_key`, shared across every game on that console (PS2/PCSX2), single generation (newest-wins overwrite), bytes on disk under `blobs/console_saves/<key>`; store methods `push_console_save_file`/`pull_console_save_path`/`get_console_save_meta` mirror the per-game save methods but key by console — issue #295); (v12 adds `console_defs.rom_extensions` — a console's scannable ROM extensions decoupled from core-derived `systemKeys`, so a standalone-only console with no libretro core (PS2) still scans the right files without surfacing PS1's shared-extension disc cores; and `standalone_emulators.launch_args` so PCSX2 boots with `-batch -fullscreen`. `get_console_defs` returns `romExtensions` (falling back to `systemKeys` when unset) and the scan uses it; `cli/consoles_data.py` adds the `ps2`/PCSX2 (`_PCSX2`) def — issue #293); (v11 adds `standalone_emulators.dirs_json` — an extensible per-emulator JSON blob of `~`-templated dir paths keyed native/flatpak → save/state/memcard, so standalone emulators like PCSX2 carry their state + memory-card dirs; the seed (`cli/consoles_data.py`) now actually emits each console's `standalones` (previously hardcoded `[]`, so they never reached the DB and mGBA was unselectable) and `console_defs.py` round-trips them via `_standalone_row_to_dict` — issue #292); (v10 adds the network-ROM source columns, issue #255: `consoles.device_network_folder`/`device_local_folder` for this device's per-console NAS mount + local-copy destination, and `game_devices.rom_source`/`rom_rel_path`/`local_rom_path`/`rom_sha256` — existing rows default to `rom_source='local'`); (v9 adds `save_conflicts` for the GUI Conflicts panel via `ConflictMixin` — `add_conflict`/`list_open_conflicts`/`dismiss_conflict`, issue #243); (v8 moves save/state bytes out of SQLite onto disk, issue #239; v7 adds `rom_transfers.sha256` for transfer integrity, issue #214; v6 added `console_key` to `core_defs`); `blobs.py` (`SaveStateMixin`) stores blob **bytes on disk** under `<data_dir>/blobs/<saves|states>/<row id>` — only metadata (`hash`/`pushed_at`/`size`/`device_id`) lives in the `saves`/`states` rows (issue #239), so the DB stays small even with large state archives × `HISTORY_LIMIT` generations. It also **keeps history**: `_push_blob`/`_push_blob_file` append a new generation instead of overwriting (deduping identical consecutive content) and `_prune_history` prunes to the newest `HISTORY_LIMIT` (20) per game, unlinking the pruned files too; the *current* blob is the most recently inserted row (ordered by `rowid` DESC, monotonic for inserts). `push_save_file`/`pull_save_path` (+ state equivalents) let the API stream uploads to a staged temp file (`new_upload_path()`) and serve pulls via `FileResponse` without buffering whole blobs in memory; `remove_game` calls `delete_blobs_for_game` to unlink a game's files before the FK cascade drops its rows; `remove_game_device(slug, device_id)` (issue #343) instead deletes just one `game_devices` row, leaving the game, its saves/states, and every other device's config alone. `list_save_history`/`restore_save` (+ `state` equivalents) back the rollback feature, and `restore` re-inserts a past version's bytes as a new generation so history only grows forward; `integrity_for_game(slug)`/`sweep_integrity()` + the private `_classify_blob`/`_last_good_version` classify a game's *current* save/state blob as ok/damaged/missing (damaged = 0-byte OR shrank below `_SHRINK_FLOOR`=0.5 of the prior generation OR on-disk bytes ≠ recorded hash OR file gone) and surface the newest healthy generation to restore — computed from existing metadata + the blob file, no schema column (issue #285); `ensure_device()` returns `(device, is_new)` tuple to signal first-time registrations; `rom_transfers` tracks pending ROM file deliveries (with a `sha256` of the staged file); `rom_pull_requests` tracks pending pull requests (receiver asks source to send); `console_defs`/`system_defs`/`core_defs`/etc. store global emulator/console definitions seeded by `cli/server.py` on startup (data from `cli/consoles_data.py`); `upsert_console_for_game(store, device_id, console_name, rom_path, save_path, rom_folder_path)` is a module-level helper (called by both `server/api/games.py` and `cli/game.py`) that infers emulator/folder paths from game paths and creates-or-updates the `Console` row; `settings.py` (`SettingsMixin`) backs the above with the generic `server_settings` key-value store (v14) |
...
| `server/sync_client.py` | HTTP client wrapping all server endpoints (used by `emusync run`, `push`, `pull`); uses a persistent `httpx.Client` for connection pooling (keep-alive); sends PIN + device headers for auth; `GameDeviceConfig` holds `rom_path`, `save_path`, `launch_command`, `state_path`, `rom_folder_path`, `rom_source`, `rom_rel_path`, `local_rom_path`, `rom_sha256`, `device_network_folder`, `device_local_folder` (the last two are transient — they populate the console row's per-console network/local folder config when passed to `set_game_device`); `list_my_game_devices()`, `list_device_games()`, `get_device_consoles()`, `create_rom_transfer()`, `create_pull_request()`, `list_pending_pull_requests()`, `complete_pull_request()` support the push/pull flow; `download_transfer()` verifies the download against the server's `X-Rom-Hash`/recorded SHA256 and raises (deleting the partial file) on mismatch (issue #214); `list_save_history()`/`restore_save()` (+ `state` equivalents, all routed through the private `_list_history`/`_restore` helpers — issue #240) back the history/rollback feature; `get_console_memcard_meta()`/`pull_console_memcard()`/`push_console_memcard()` are the console-scoped shared-card methods (keyed by console abbr) the PS2 `_MemcardClient` adapter drives — issue #295; both file-based and folder-based memcards (PCSX2 `.ps2` directories) are supported. **PCSX2 nests each game's saves one level down in its own subfolder** (e.g. `GAME1/GAME1`, `GAME1/icon.sys`) — `memcard_bytes(card_path)` walks the *whole tree* (`rglob`, not a single-level `iterdir`) and serialises it as a deterministic plain tar (sorted, mtime=0, relative paths preserved) so the SHA-256 is stable for hash comparison; a top-level-only walk silently drops every game's data (issue #320). `_write_memcard(card, data)` detects tar archives on pull, backs up the **entire existing memcard folder** as `<card>.bak` (a `shutil.copytree` sibling) before extracting — not per-file `.bak` suffixes inside the folder — and extracts through `_safe_extract_tar` (the same path-traversal guard used for state archives); falls back to a raw-file write for a legacy file-based memcard. The `memcard:push`/`memcard:pull` IPCs mirror this: push packs with `tar -cf`, pull probes with `tar -tf`, backs up the whole folder with `cp -r` to `<name>.bak`, then extracts with `tar -xf`. |
| `gui/electron/` (main process) | Split into per-domain modules (issue #222) — `main.ts` is now just IPC registration + app lifecycle (calls each module's `register*Ipc()`). Shared mutable state (server/game/daemon process handles, the window, console-def caches) lives on the single `rt` object in `runtime.ts` (ES modules only export read-only bindings, so cross-module reassignment needs an object). Modules: `runtime.ts` (constants `CONFIG_PATH`/`SCRIPT`/`PYTHON` + `rt`); `http.ts` (`httpGetJSON` — main process must use Node http, not `fetch`); `window.ts` (`createWindow`); `config-store.ts` (`config:*` IPC + `loadServerCfg`); `server.ts` (server + sync-daemon lifecycle, `server:*`/`daemon:*` IPC, `startServerProcess`/`killServerByPid`/`killOrphanServers`); `game.ts` (`game:*` IPC + `launcher:path`); `files.ts` (dialogs, `files:*`, `device:probe`, `findLatestFileInDir`); `sync.ts` (`save:*`/`state:*`/`rom:push`/`rom:localize`/`rom:delocalize` IPC — the localize handlers do the free-space-checked, atomic, master-guarded local copy and update the server game config, issue #255); `emulator/{types,console-defs,detect,scan,ipc}.ts` (the import-wizard subsystem — `emulator:consoles`/`detect`/`scan` lazily fetch console defs from the Python API); `steamgriddb.ts` (`getSteamGridDbKey()` + `steamgriddb:getKey`/`setKey`/`openKeyPage` IPC — fetches/sets the shared key via the server API, and opens the browser to `steamgriddb.com/profile/preferences/api`; `art.ts`'s `art:get` tries SteamGridDB first when a key is configured, falling back to the existing libretro-thumbnails exact-match lookup, issue #322); `artwork.ts` (`artwork:searchGames`/`listCandidates`/`setArt`/`clearArt`/`getCurrent`/`refreshAll` IPC backing the per-game Artwork tab, issue #325 — reuses `art.ts`'s exported `ART_DIR`/`ART_TYPES`/`download`/`getSgdbImagesForType`/`makeSteamGridDbClient`/`toDataUrl` rather than duplicating the type→SGDB-method branch or the ESM/CJS interop workaround). Shared emulator types (`DetectedEmulatorOption`, `EmulatorScanResult`, `RomEntry`) live in `emulator/types.ts` and are type-imported by `preload.ts` |
...
| `gui/renderer/src/gameDelete.ts` | `deleteGame(slug, {deleteLocalRom, removeEverywhere})` (issue #343) — the tiered delete shared by `GameConfig.tsx`'s single-game delete and `GameGrid.tsx`'s bulk delete. Tier 1 (always): `removeGameDevice` unlinks just this device. Tier 2 (`deleteLocalRom`): also deletes the ROM from this device's disk — `rom:delocalize` for a network ROM's localized copy (has its own empty-dir cleanup), `rom:deleteFile` for a local-source ROM. Tier 3 (`removeEverywhere`): also deletes the network-share master (if network-sourced) then calls the full-purge `removeGame` (every device, save/state history, blobs) |
| `gui/renderer/src/time.tsx` | Timestamp formatting (issue #216): `formatRelative()` + the `<RelTime>` component render a relative phrase ("2 hours ago") with the exact local 12-hour time in a hover tooltip. **All EmuSync timestamps are UTC** (server `isoformat()`, Electron `toISOString()`), and a tz-less ISO string parses as *local* in JS — so `parseUtc()` appends `Z` when no offset/`Z` is present. Used by SaveHistory, DevicesButton, ServerStatusButton, GameConfig |
| `gui/renderer/src/App.tsx` | Root component; screen router; auto-starts server if `is_server=true`; on the console screen the topbar shows the console abbr/label/game-count (reusing `GameGrid`'s `.game-grid-abbr`/`-label`/`-total` classes) next to the "‹ Back" link — grouped in one flex item since `.topbar` uses `justify-content: space-between` (issue #349); this "‹ Back" link (issue #351) is the only back control now — `GameGrid` dropped its own redundant "‹ Back" button (issue #350); the mouse's back/forward side buttons also work (`MouseEvent.button === 3`/`4`) via a single always-registered `mouseup` listener driven by refs (not state, so it isn't re-subscribed every screen change): back mirrors "‹ Back" (console -> games), forward re-enters whichever console was most recently left, one level deep — there's no multi-page history, just `games` <-> `console` (issues #354, #356) |
...
| `gui/renderer/src/components/ConsoleGrid.tsx` | Home screen: grid of console cards (one per imported console). Each card shows the console icon, abbr, label, and game count with a colour-coded accent. Clicking drills into that console's game grid. Fetches the console-def list via `emulator:consoles` to map stored abbr → key (issue #304) |
| `gui/renderer/src/components/GameGrid.tsx` | Per-console game grid. Shows all games for the selected console as `GameCard` tiles; splits into "On this device" and "On other devices" sections; has a live search filter, a `GameFilterButton` filter popover (issue #345), and an artwork-type dropdown (grid/wide_grid/hero/logo/icon, persisted to `art_type_by_console` in config — issue #324, wide_grid added #333, changing it remounts every visible `GameCard` via a `key` that includes the type so they re-fetch); has no back button of its own (dropped as redundant with the topbar's "‹ Back" link, issues #350/#351) or `consoleLabel`/`consoleAbbr` props (that display moved to `App.tsx`'s topbar, issue #349); opens `GameModal` and `NetworkPlaySetup` as needed (issue #304); multi-select bulk delete offers the same tiered `deleteGame` checkboxes as `GameConfig.tsx`, applied uniformly to every selected game (issue #343); a Select All / Deselect All button (`toggleSelectAll`) sits left of Delete and toggles selection over `filtered` — the currently visible games after search + `GameFilterButton` — not the console's full list (issue #353); both header buttons are always rendered (disabled/greyed when not applicable) rather than appearing/disappearing, sized to match the dropdown/search box model via the shared `.game-grid-header-btn` (issue #347) |
| `gui/renderer/src/components/GameFilterButton.tsx` | Filter popover next to `GameGrid`'s search box (issue #345): 3 checkbox groups — Artwork (with/without), Saves (on device/not on device), ROM availability (localized/not localized) — combining OR-within-group, AND-across-groups via the exported `matchesFilters`. "Localized" checks whether the ROM's bytes are actually on this device's disk right now (`romSource !== "network" \|\| hasLocalCopy`), not just whether the game is network-sourced — a network game with a local copy still counts as localized. Saves reads straight off `GameRow.lastSave` (already populated); artwork has no such field, so `GameGrid` lifts each `GameCard`'s own `art:get` result via a new `onArtStatus` callback into a `hasArt` map instead of a duplicate fetch — a game not yet resolved (`undefined`) passes the artwork filter rather than being hidden while its card loads. Closes on outside click; badges the button with the active filter count |
| `gui/renderer/src/components/GameCard.tsx` | Individual game card: fetches artwork via `art:get` IPC (SteamGridDB primary, libretro-thumbnails fallback for grid type), falls back to a styled colour-gradient placeholder, and reports the fetch result up via the optional `onArtStatus(slug, hasArt)` prop for `GameFilterButton`'s artwork filter (issue #345); `artType` prop (from `GameGrid`'s per-console dropdown) picks the tile's CSS treatment (`game-card-art-{grid,wide_grid,hero,logo,icon}`) since Hero/Logo/Icon aren't boxart-shaped — issue #324, wide_grid added #333; clicking anywhere on the card opens the settings modal (`onSettings`), except the ▶ play button and the selection checkbox, which `stopPropagation` (issue #337 — replaced a standalone ⚙ button); shows status badges (locked, other-device, network ROM) (issue #304) |
| `gui/electron/art.ts` | `art:get` IPC handler: checks `~/.emusync/art/<consoleKey>/<slug>/<type>.png` cache (one folder per console, one subfolder per game, one file per artwork type — `type` ∈ grid/wide_grid/hero/logo/icon, resolved per-console from `art_type_by_console` in config, defaulting to `grid`; issue #324, wide_grid added #333), then tries SteamGridDB (`fetchFromSteamGridDb` — resolves which SGDB game to use via the shared `resolveSgdbGameId` (also used by `artwork:refreshAll`'s fallback): reuses `sgdb_game_id` if the game already has one (manually picked, or a prior auto-match), otherwise fuzzy-searches via `searchGame` and **persists the top result as `sgdb_game_id`** via `PUT /games/{slug}` so it becomes the game's "first choice" and every later fetch — on this device or any other — reuses the same match instead of re-searching and potentially landing on a different game (issue #339); a manual pick in the Artwork tab always overwrites it and wins. Then `getSgdbImagesForType` (the shared per-type SGDB dispatch — grid calls `getGrids` filtered to `dimensions: ["600x900"]`; wide_grid calls `getGrids` with no dimension filter and instead keeps whichever results have `width > height`, since SteamGridDB's landscape grid art comes in several sizes and an exact-dimension filter under-populated results, issue #341; hero/logo/icon call their own SGDB methods) fetches the resolved type, using the key from `steamgriddb.ts`'s `getSteamGridDbKey()`; no-op if no key configured) before falling back, **grid type only**, to the exact-filename `raw.githubusercontent.com/libretro-thumbnails/<System>/master/Named_Boxarts/<GameName>.png` lookup (wide_grid/hero/logo/icon have no libretro equivalent, so those just show the placeholder on a miss); atomic temp→rename write; returns `file://` URL or null (issues #304, #322, #324) |
| `gui/renderer/src/components/NetworkPlaySetup.tsx` | Play-time cross-device setup modal (issue #270) — for a game not set up on this device, offers (A) point this device at the same network share (folder-picks a mount root → `rom:setupNetworkPlay` → verifies + writes a network config → plays) or (B) pull the ROM from a device that has it via `createPullRequest` (delivered by the source's sync-daemon). Reads `GET /games/{slug}/network-source` + `/games/{slug}/devices` |
| `gui/renderer/src/components/ConflictsButton.tsx` | Top-bar "⚠ Conflicts" panel (issue #243) — polls `GET /conflicts`, lists auto-resolved save divergences, and recovers the losing copy by finding it in `save/history` by hash and restoring it (+ `save:pull` if the game is local); shows the `.bak` guidance when the loser isn't on the server (server-won case). Only renders when there are open conflicts |
| `gui/renderer/src/components/SaveHistory.tsx` | Per-game save/state recovery modal (issues #7 + #285) — merges server save **and** state history generations with this device's local `.bak` losers into one chronological, kind-tagged timeline; flags a damaged current blob (⚠, from `GET /games/{slug}/integrity`) with a one-click "Restore last good" (`last_good_version_id`); a normal row Restore makes a version current on the server and (if local) writes it to disk via `save:pull`/`state:pull`; a `local-bak` row recovers a copy that may never have reached the server via `recovery:restoreLocalBackup`. Suggest-only, never auto-acts. Takes `savePath` + `statePath` props (threaded through `GameModalTarget`/`GameRow`) |
| `gui/renderer/src/components/GameConfig.tsx` | Add/edit game form with file pickers; Delete confirm offers the tiered `deleteGame` options (issue #343) — two checkboxes ("delete ROM from local folders", "remove from all devices and delete network ROM") on top of the always-happening this-device unlink |
| `gui/renderer/src/components/GameModal.tsx` | Tabbed per-game modal (issue #260): Settings (`GameConfig`), Artwork, Devices, Save history, Run. Owns the active-tab state; each tab is a separate embedded component |
| `gui/renderer/src/components/ArtworkTab.tsx` | The Artwork tab (issue #325): search SteamGridDB (`artwork:searchGames`) → scrollable results list (~4 rows) → picking a result persists `sgdb_game_id` via `setGameSgdbId` (shared across devices, issue #325's schema addition); 5 current-artwork tiles (grid/wide_grid/hero/logo/icon, from `artwork:getCurrent`, read-only cache lookup); clicking a tile opens a picker modal (`artwork:listCandidates`, thumbnails loaded directly via `<img src>` from SteamGridDB's CDN rather than round-tripping through Electron) — if no match has been picked/resolved yet, it first calls `artwork:resolveMatch` to fuzzy-search-and-persist one on demand rather than blocking with "search first" (issue #339 follow-up, covers art cached before that persistence existed) — with a red-× to clear the saved art (`artwork:clearArt`) and click-to-replace (`artwork:setArt`); one Refresh-all button (`artwork:refreshAll`) re-fetches all 5 types bypassing the cache. On mount, an already-set `sgdb_game_id` is resolved to its name via `artwork:getMatchedGame` and shown as a persistent "✓ Matched: <name>" line next to the search box — the results list itself starts empty on every mount, so without this the persisted pick looked reset on reopen (issue #335). Derives `consoleKey` from `gameConsole.toLowerCase()` rather than threading a separate prop through `GameModal`/`GameGrid` — every seeded console's abbr lowercases to its key |
| `gui/renderer/src/components/ConsoleImport.tsx` | "Add Console" wizard modal — console dropdown → emulator detection → ROM scan → import. Thin shell: runs the `useConsoleImport` state machine and renders the step component for the current phase (issue #229). The pieces live in `components/console-import/`: `types.ts`, `helpers.ts` (pure, testable: `slugify`/`getConsoleAbbreviation`/`resolveRomFolder`/`annotateRoms`/`dedupeAndLink`/`groupByDir`/`classifyByRoot`), `useConsoleImport.ts` (all state + async handlers; returns a `ConsoleImportVM` object the steps consume). Network-source imports (issue #270) also scan the console's local-copy folder so a ROM already on local disk is detected — `classifyByRoot` tags each as network-only / local-only / both; a local-only ROM is uploaded to the share via `rom:uploadMaster` and registered as an already-localized network game (`local_rom_path` = the existing local file), and a "both" ROM is registered with its local copy already linked; after import, `pullFromServerIfNewer` pulls any save/state (or, for a shared-layout console, the console memory card) that's already on the server and newer than what's local, so a device importing a game another device already played doesn't start from an empty save (issue #316); and one component per phase (`ConsoleStep`/`EmulatorStep`/`ResultsStep`/`DoneStep` + shared `Stepper`/`Spinner`) |
...
window.emusync.files.getPs2LastPlayed()      // {slug: ISO} — PS2 per-game last-played, joining ~/.emusync/ps2_serials.json (slug→serial, learned by `emusync run`) with PCSX2's live playtime.dat (serial→last-played); fills the gap left by the shared memory card (issue #301)
window.emusync.files.renameGameFiles({romPath, savePath, stateFolder, newBase, reorganize, secondaryRomPath?}) // renames a game's ROM file (+ optional secondary copy e.g. a network ROM's local copy) and its save file/state folder to `newBase` (the sanitized title); when reorganize=true a flat ROM is nested under <dir>/<newBase>/, else renamed in place; save/state targets are recomputed from newBase under the same roots and the current/legacy files migrated into them; best-effort no-op when a source is missing or already at target; returns {ok, newRomPath, newSavePath, newStateFolder, newSecondaryRomPath?, error?} (issue #283 — clean-up-on-import; generalises the old move-to-subfolder)
...
window.emusync.memcard.push(consoleKey, cardPath) // reads the local card file, POSTs bytes to /consoles/{key}/memcard; backs the manual "Push memory card" button in GameConfig for shared-layout consoles (issue #319) — the only other push path is emusync run's automatic post-launch push; returns {ok, error?}
window.emusync.memcard.pull(consoleKey, cardPath) // GETs /consoles/{key}/memcard, backs up existing file to .bak, writes new bytes; used by the console-import wizard to bring a shared-layout console's card in from the server right after import (issue #316), and by GameConfig's manual pull button (issue #319); returns {ok, pulled, error?}
...
window.emusync.rom.localize(slug, destFolder?)          // copy a network ROM onto local disk for offline play (free-space precheck, atomic temp+rename, sha256, updates server config); destFolder used only when no local_rom_path is stored; returns {ok, localPath?, error?} — issue #255
window.emusync.rom.delocalize(slug)                     // delete the localized copy (never the NAS master) + clear local_rom_path/rom_sha256; returns {ok, error?} — issue #255
window.emusync.rom.uploadMaster(localPath, networkPath) // copy a local-only ROM UP to the network share so it becomes the master (skip if a master already exists; free-space precheck, atomic temp+rename, sha256 verify; never overwrites an existing master); returns {ok, sha256?, skipped?, error?} — issue #270
window.emusync.rom.setupNetworkPlay(slug, mountRoot)    // for a game on a network share configured by another device, point THIS device at the same share: verify join(mountRoot, rom_rel_path) exists, derive local save/state paths from this device's console folders, write a network-source game config; returns {ok, romPath?, error?} — issue #270
window.emusync.rom.deleteFile(absolutePath)              // bare unlink of an absolute path, no server-config coupling; used by the tiered game-delete flow (gameDelete.ts) for a local-source ROM and a network master — a network ROM's localized copy instead goes through rom:delocalize, which already has empty-dir cleanup; returns {ok, error?} — issue #343

window.emusync.recovery.listLocalBackups(savePath, stateFolder) // enumerate on-disk .bak losers for the recovery view: the save's `<save>.bak` + the state folder's `*.bak` (with size/mtime); fs touches are guarded so a dead mount returns empty, never hangs; returns {saves: LocalBak[], states: LocalBak[]} — issue #285
window.emusync.recovery.restoreLocalBackup(bakPath, targetPath) // restore a .bak loser back into place (reads bak bytes first to avoid the save self-overwrite race, atomic temp+rename, Windows-safe); local-only, no server push; returns {ok, error?} — issue #285
...
window.emusync.art.get(slug, gameName, consoleKey) // fetch this console's configured artwork type for a game: checks ~/.emusync/art/<consoleKey>/<slug>/<type>.png first; on miss, tries SteamGridDB, then (grid type only) the public libretro-thumbnails GitHub repo (Named_Boxarts/<GameName>.png, keyed by consoleKey → libretro system name); caches to disk; returns a file:// URL or null when no art is available (issues #304, #324)
...
window.emusync.artwork.searchGames(name)                       // SteamGridDB searchGame — candidate game matches for the Artwork tab's results list (issue #325)
window.emusync.artwork.getMatchedGame(sgdbGameId)               // SteamGridDB getGameById — resolves the name of an already-picked sgdb_game_id so the Artwork tab shows "✓ Matched: <name>" on reopen, independent of the (initially empty) search-results list (issue #335)
window.emusync.artwork.resolveMatch(slug, gameName)             // on-demand fuzzy-search-and-persist (wraps art.ts's resolveSgdbGameId) for a game with no sgdb_game_id yet — called from the picker modal so a game whose art was cached before #339's persistence existed doesn't just get told to search manually (issue #339 follow-up)
window.emusync.artwork.listCandidates(sgdbGameId, type)         // all available images of one type (grid/wide_grid/hero/logo/icon) for a SteamGridDB game — powers the picker modal
...
window.emusync.artwork.refreshAll(slug, gameName, consoleKey, sgdbGameId) // re-fetches all 4 types fresh (bypasses cache), using sgdbGameId if given else searches by name first
```
...
| `~/.emusync/ps2_serials.json` | `{slug: serial}` map for PS2 games, learned by `emusync run` from PCSX2's `playtime.dat` at session exit (`_learn_ps2_serial`); joined with the live `playtime.dat` by the `files:get-ps2-last-played` IPC so the GUI shows a per-game last-played despite the shared card (issue #301) |
| `~/.emusync/art/<consoleKey>/<slug>/<type>.png` | Cached artwork for a game, one file per SteamGridDB asset type (`grid`/`wide_grid`/`hero`/`logo`/`icon`) so switching a console's artwork type never deletes another type's cached file; downloaded once from SteamGridDB or (grid only) the libretro-thumbnails GitHub CDN by the `art:get` IPC handler; used by the game grid view (issues #304, #322, #324, #333) |
...
Config fields: `server_host`, `server_port`, `data_dir`, `device_id`, `device_name`, `is_server`, `server_pin` (optional — blank = open access), `recent_import_folders` (dict mapping console keys to lists of recent folder paths), `watch_saves` (optional bool — opt-in background save/state watcher in `sync-daemon`, issue #242), `import_rom_source` (dict console-key → `"local"`/`"network"`, the remembered import source per console) and `import_local_folder` (dict console-key → local-copy destination folder) — both issue #255; `art_type_by_console` (dict console-key → `"grid"`/`"wide_grid"`/`"hero"`/`"logo"`/`"icon"`, this device's chosen SteamGridDB asset type per console, defaulting to `"grid"` when unset — issue #324; `wide_grid` — SteamGridDB's landscape "header capsule", 460x215 — added issue #333).
...
- You added a new API route (in the relevant `server/api/` router module) → add an integration test for the happy path and the main error case (404, 403, 409, etc.)
...
**Shared-save-layout consoles (PS2) — never treat the save/state as per-game** — For consoles where the save (and states) live in ONE shared location across every game (PS2's `memcards/Mcd001.ps2` + `sstates/`, issues #294/#295), the per-game save/state must not be renamed, moved, or pushed per-game. The renderer gates this via `usesSharedSaveLayout(consoleKeyOrAbbr)` in `console-import/helpers.ts` (currently `{ps2}`): the import (`useConsoleImport`) passes empty save/state to `renameGameFiles` and skips the per-game `autoPush` of save/state; `GameConfig` skips the save/state rename on a settings name-change and hides the manual **per-game** save/state sync panel (a per-game state *pull* there would be destructive — `_extract_state_folder` `.bak`s every game's slots) — but shows a console-scoped memcard push/pull `SyncLine` instead (issue #319), since unlike per-game saves, pushing/pulling the whole shared card on demand is safe and sometimes necessary (e.g. a card edited outside EmuSync, with no play session to trigger the automatic sync). The CLI mirror is `run.py`'s `_SHARED_MEMCARD_CONSOLES`/`_SHARED_STATE_CONSOLES` and `scan.ts`'s `SHARED_MEMCARD`. Keep all four in sync if you add a shared-layout console. **Import-time pull (issue #316):** since nothing else populates a shared card on a device seeing this console for the first time, `useConsoleImport`'s `pullFromServerIfNewer` pulls the console-scoped card via `window.emusync.memcard.pull` right after import if the server's copy is newer (or local has none) — the counterpart to `autoPush`'s per-game push, but console-keyed and pull instead of push. Shared save *states* are deliberately NOT pulled at import time — they're keyed by game serial with no console-scoped endpoint, so only `emusync run`'s per-launch serial-filtered sync touches them.

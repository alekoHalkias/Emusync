...
**DB schema initialization — split statements, avoid `executescript()`** — `store.py` initializes tables by splitting `_SCHEMA` and executing each statement individually with explicit `commit()` after each statement. `executescript()` is deprecated and incompatible with WAL mode, causing `sqlite3.InterfaceError` on fresh database creation. Commits between statements are required for proper isolation when the connection is used from multiple threads (via `check_same_thread=False`). Use `try/except` for `INSERT vs UPDATE` instead of `ON CONFLICT` clauses for broader sqlite3 compatibility.

...
**`switch_title_id` can now also be discovered by name, not just the bracket-tag filename or post-launch self-heal (#448).** `gui/electron/switchtitledb.ts` matches a game's display name against blawar/titledb's public catalog (cached to `~/.emusync/switch_titledb.json`, exact-match only — a fuzzy guess risks seeding a save into the wrong title's Eden folder). Runs at GUI import time, once per app session for every already-imported Switch game still missing an ID (`App.tsx`'s `backfillAllSwitchSaves`), and via a manual "Look up title ID" button on `SwitchModsTab.tsx`.

**`ensure-switch-save-folder` seeds a real save before falling back to a placeholder, and runs for every known-title-ID Switch game every session, not just newly-discovered ones (#453, on top of #443/#448).** Whenever a title ID is known — freshly discovered above, or already set from an earlier session/device — `emusync game ensure-switch-save-folder <slug>` first checks `_find_local_switch_save`; if this device already has a real save, nothing is touched. Otherwise it tries `_seed_switch_save` (the same helper `emusync run` uses to pre-seed a first-ever session) so a device that pairs *after* another device already has progress doesn't have to wait for its own first launch to receive it. Only when the server has nothing to seed either does it fall back to #448's original empty `<profile>/<title-id>/` placeholder. `App.tsx`'s per-session sweep calls this for every Switch game with a known ID, not only ones it just resolved a name lookup for — otherwise a device that joins later than the ID's discovery never gets its own folder/seed until it happens to launch the game itself.

**Switch save resolution is restricted to one known Eden profile per device once it has one, not re-scanned across every local profile on every call (#455).** `cli/run_switch.py`'s `_known_switch_profile_root(client, device_id)` reads `consoles.device_save_folder` for this device's `"Switch"` row — a column that already gets set for free by `upsert_console_for_game` every time `set_game_device` persists a `save_path` (`Path(save_path).parent`, same mechanism the shared-save-layout consoles below use for their card root), no separate storage needed. `_find_local_switch_save`, `_seed_switch_save`, and `ensure-switch-save-folder` (#453) all take this as an optional `known_root`/`profile_root` and, once one is known, restrict themselves to just that folder instead of every folder `_switch_profile_dirs()` finds. Without this, a device that happens to have more than one local Eden profile folder (a stray leftover, a guest profile Eden created for some other reason) could get a save matched against — or seeded into — the wrong one, which looks exactly like "it pushed my save into some other profile I don't use." Only the very first-ever resolution on a device (no known root yet) still scans every local profile, same as before — that first successful match/seed is what causes `device_save_folder` to get stamped for every call after.

...
**DB schema initialization — split statements, avoid `executescript()`** — `store.py` initializes tables by splitting `_SCHEMA` and executing each statement individually. `executescript()` is deprecated and incompatible with WAL mode, causing `sqlite3.InterfaceError` on fresh database creation. Use `try/except` for `INSERT vs UPDATE` instead of `ON CONFLICT` clauses for broader sqlite3 compatibility.

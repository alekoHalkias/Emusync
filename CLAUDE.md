...
**DB schema initialization & thread safety** — `store.py` wraps database initialization (PRAGMA and schema setup) with `threading.Lock()` to prevent concurrent access issues when uvicorn worker threads access the Store instance simultaneously. The lock serializes PRAGMA execution and schema creation to avoid `sqlite3.InterfaceError`. Each statement is executed and committed individually because `executescript()` is deprecated and incompatible with WAL mode.

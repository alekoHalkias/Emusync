...
| `gui/renderer/src/components/GameGrid.tsx` | Per-console game grid. Shows all games for the selected console as `GameCard` tiles; splits into "On this device" and "On other devices" sections; has a live search filter and an artwork-type dropdown (grid/hero/logo/icon, persisted to `art_type_by_console` in config — issue #324, changing it remounts every visible `GameCard` via a `key` that includes the type so they re-fetch); back button returns to `ConsoleGrid`; opens `GameModal` and `NetworkPlaySetup` as needed (issue #304) |
| `gui/renderer/src/components/GameCard.tsx` | Individual game card: fetches artwork via `art:get` IPC (SteamGridDB primary, libretro-thumbnails fallback for grid type), falls back to a styled colour-gradient placeholder; `artType` prop (from `GameGrid`'s per-console dropdown) picks the tile's CSS treatment (`game-card-art-{grid,hero,logo,icon}`) since Hero/Logo/Icon aren't boxart-shaped — issue #324; shows play/settings buttons and status badges (locked, other-device, network ROM) (issue #304) |
| `gui/electron/art.ts` | `art:get` IPC handler: checks `~/.emusync/art/<consoleKey>/<slug>/<type>.png` cache (one folder per console, one subfolder per game, one file per artwork type — `type` ∈ grid/hero/logo/icon, resolved per-console from `art_type_by_console` in config, defaulting to `grid`; issue #324), then tries SteamGridDB (`fetchFromSteamGridDb` — fuzzy title search via `searchGame` + the matching `getGrids`/`getHeroes`/`getLogos`/`getIcons` call for the resolved type, using the key from `steamgriddb.ts`'s `getSteamGridDbKey()`; no-op if no key configured) before falling back, **grid type only**, to the exact-filename `raw.githubusercontent.com/libretro-thumbnails/<System>/master/Named_Boxarts/<GameName>.png` lookup (hero/logo/icon have no libretro equivalent, so those just show the placeholder on a miss); atomic temp→rename write; returns `file://` URL or null (issues #304, #322, #324) |
...
window.emusync.artwork.listCandidates(sgdbGameId, type)         // all available images of one type (grid/hero/logo/icon) for a SteamGridDB game — powers the picker modal
...
| `~/.emusync/art/<consoleKey>/<slug>/<type>.png` | Cached artwork for a game, one file per SteamGridDB asset type (`grid`/`hero`/`logo`/`icon`) so switching a console's artwork type never deletes another type's cached file; downloaded once from SteamGridDB or (grid only) the libretro-thumbnails GitHub CDN by the `art:get` IPC handler; used by the game grid view (issues #304, #322, #324) |

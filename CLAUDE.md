...
| `gui/renderer/src/components/GameGrid.tsx` | Per-console game grid. Shows all games for the selected console as `GameCard` tiles; splits into "On this device" and "On other devices" sections; has a live search filter; back button returns to `ConsoleGrid`; opens `GameModal` and `NetworkPlaySetup` as needed (issue #304) |
| `gui/renderer/src/components/GameCard.tsx` | Individual game card: fetches boxart via `art:get` IPC (libretro thumbnails), falls back to a styled colour-gradient placeholder; shows play/settings buttons and status badges (locked, other-device, network ROM) (issue #304) |
| `gui/electron/art.ts` | `art:get` IPC handler: checks `~/.emusync/art/<slug>.png` cache, then tries SteamGridDB (`fetchFromSteamGridDb` — fuzzy title search via `searchGame`/`getGrids`, using the key from `steamgriddb.ts`'s `getSteamGridDbKey()`; no-op if no key configured) before falling back to the exact-filename `raw.githubusercontent.com/libretro-thumbnails/<System>/master/Named_Boxarts/<GameName>.png` lookup; atomic temp→rename write; returns `file://` URL or null (issues #304, #322) |
...
| `~/.emusync/art/<slug>.png` | Cached boxart for a game, downloaded once from the libretro-thumbnails GitHub CDN by the `art:get` IPC handler; used by the game grid view (issue #304) |

...
window.emusync.memcard.pull(consoleKey, cardPath) // GETs /consoles/{key}/memcard, backs up existing file to .bak, writes new bytes; used by the console-import wizard to bring a shared-layout console's card in from the server right after import (issue #316) — emusync run remains the only thing that PUSHES it; returns {ok, pulled, error?}

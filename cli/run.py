...
def run_game(game_slug: str, command: tuple[str, ...]) -> None:
    ...
    try:
        ...
        # Wii and Switch instead adopt whichever NAND/title save folder was
        # actually written, since the save location isn't knowable until a
        # game has been played at least once (#431, #419).
        if shared_memcard:
            ...
            actual_state_path = None
        elif console_abbr == "Switch":
            actual_save_path = _resolve_written_switch_save(launch_start)
            actual_state_path = None
        else:

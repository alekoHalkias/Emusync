...
# SD-card title tree (#418). Derived from the same `shared_memcard`/
# `shared_state` flags in cli/consoles_data.py that seed console_defs
# (issue #490) — the GUI reads that same data over the /console-defs API
# (gui/electron/emulator/scan.ts, console-import/helpers.ts) instead of each
# maintaining its own independent copy, which had already drifted out of sync
# between the two.
_SHARED_MEMCARD_CONSOLES = {c["abbr"] for c in _IMPORT_CONSOLES if c.get("shared_memcard")}
...
_SHARED_STATE_CONSOLES = {c["abbr"] for c in _IMPORT_CONSOLES if c.get("shared_state")}

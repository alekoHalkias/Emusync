...
### Switch update/DLC management (`emusync update`, issue #441)

Eden has no headless install path for update/DLC `.nsp` files (GUI-only "File > Install Files to NAND", requiring the user's own prod.keys), and its installed content lives in one shared content-addressed NAND store (`nand/system/Contents/registered`), not a per-title folder — so it can't be synced like a save/state. Instead this manages the not-yet-installed files themselves, reusing the ROM-transfer wire protocol almost entirely as-is (it already accepts an arbitrary filename + informational destination path per game/device, not hardcoded to "the ROM"): `rom_transfers` gained a `kind` column (`'rom'` default, or `'update'`) so the receiving side routes an update transfer into a managed per-game folder instead of overwriting `rom_path`. Only the *filename* is taken from the sender's `destination_path` (its basename) — the receiving device always computes its own local managed folder rather than trusting a sender-supplied absolute path for this kind (mirrors the `~`-expanded-client-side pattern used for standalone emulator dirs, #292).

**Scope is push-only** — no pull-request support for updates (that would need `rom_pull_requests`/the sync-daemon's auto-fulfillment handler to also carry a filename, since it currently assumes "the ROM"); deferred as a deliberate simplification since the real workflow is "download an update once, distribute it out."

- **Managed folder**: `~/.emusync/switch_updates/<slug>/` — `cli/transfer.py`'s `switch_updates_dir(data_dir, slug)` (Python) / `gui/electron/sync/update.ts`'s `switchUpdatesDir(slug)` (Electron, hardcoded to `homedir()` like `config-store.ts`'s `CONFIG_PATH`).
- **`cli/update.py`** (`emusync update` group): `add <slug> <file>` (copies into the managed folder), `list <slug>`, `push <slug> <filename>` (interactive device picker mirroring `push_rom`'s pattern, calls `create_rom_transfer(..., kind="update")`).
- **`cli/transfer.py`**: `_receive_update_transfer` (parallel to `_receive_transfer`, but never touches `game_device` registration — an update/DLC file is not the game's ROM); `_run_transfer_daemon`'s two `rom_transfer_queued` dispatch points (startup drain + SSE loop) branch on `kind` before falling through to the normal ROM-receive path.
- **API**: `POST /games/{slug}/rom-transfer` accepts an optional `X-Transfer-Kind` header (default `"rom"`), persisted and surfaced in both `/rom-transfers/pending` and the `rom_transfer_queued` SSE payload.
- **GUI**: `gui/electron/sync/update.ts` (`update:add`/`update:list`/`update:push` IPC, registered in `sync.ts`), bridged via `preload.ts`/`emusync.d.ts`. `GameModal.tsx` shows an "Updates" tab only when `consoleKey === "switch"`, rendering `UpdatesTab.tsx` (list managed files, add via `dialog.openFile`, push to another device via a per-file device dropdown).

**Auto-detection at import time**: rather than requiring every update/DLC file to be manually `add`ed, the console-import wizard detects them automatically. `cli/detect.py`'s `_find_switch_update_files(base_rom_path)` (mirrored in `gui/electron/emulator/scan.ts`'s `findSwitchUpdateFiles()`) takes the base game's title ID, drops its low 3 hex digits to get the 13-hex title prefix, and scans *only* the base ROM's own folder (non-recursive) for other `.nsp`/`.xci` files sharing that prefix — so an unrelated game's files sitting in the same flat ROM folder are never picked up. `cli/console.py`'s import step calls this per Switch base game and echoes `[N update/DLC file(s) found]` next to the save/state tags (mirrored in the GUI's `ResultsStep.tsx` as a `N update/DLC` badge); `gui/electron/emulator/scan.ts` attaches the result to each `RomEntry` as `updatePaths`.

Detected paths are recorded on `game_devices.update_paths` (`;`-joined absolute paths, migration #19 — mirrors `console_defs`'s list-field convention; `GameDevice`/`GameDeviceConfig` expose it as `list[str]`, converted to/from the joined string only at the API boundary in `server/api/games.py`). Both the manual managed-folder tool and this auto-detection stay live side by side: `cli/update.py`'s `_resolve_update_files(client, cfg, slug)` (mirrored in `gui/electron/sync/update.ts`'s `resolveUpdateFiles()`, which fetches `/games/{slug}/device` directly rather than going through the CLI) merges the managed folder with the game's recorded `update_paths` into one basename→path map before `list`/`push` — so a file the wizard found next to the ROM is pushable immediately, with no need to first copy it into the managed folder. A managed-folder entry wins on a basename collision, since adding it there was an explicit user action.

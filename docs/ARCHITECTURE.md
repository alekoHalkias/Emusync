...
| `gui/renderer/src/components/GameModal.tsx` | Tabbed per-game modal: Settings, Artwork, Devices, Save history, Mods (Switch only), Run |
| `gui/renderer/src/components/SwitchModsTab.tsx` | Switch-only Mods tab: merges the local `load/<title-id>/` listing (via `gui/electron/switchmods.ts`) with the communal server pool into one table; "Sync mods now" shells out to `emusync mod sync`, "Open mods folder" reveals it in the OS file manager (#444) |
...
| `gui/renderer/src/components/ImportWizard.tsx` | First screen shown by the topbar's import button (#462): "Add console" (renders `ConsoleImport` unchanged) vs "Import library" (renders `LibraryImport`). `NetworkPlaySetup.tsx`'s single-game repair flow renders `ConsoleImport` directly with `initialConsole` set, bypassing this choice screen entirely |
| `gui/renderer/src/components/ConsoleImport.tsx` | "Add Console" wizard modal — console dropdown → emulator detection → ROM scan → import. Thin shell running the `useConsoleImport` state machine. Pieces live in `components/console-import/`: `types.ts`, `helpers.ts` (pure/testable), `useConsoleImport.ts` (the phase state machine), `resolveRomPaths.ts` (per-ROM rename + network upload-to-share/rel-path resolution), `postImport.ts` (`prefetchArt`/`pullFromServerIfNewer`/`autoPush`). Network-source imports also scan the console's local-copy folder so an already-local ROM is detected; after import, `pullFromServerIfNewer` pulls any save/state (or, for a shared-layout console, the memory card) already on the server and newer than local. Optional `initialRomDirs`/`title` props (#462) let a caller seed the scan folder(s) directly instead of the console's persisted "recent import folders", and override the modal header — used by `LibraryImport` |
| `gui/renderer/src/components/LibraryImport.tsx` | Bulk-library import (#462): pick a local library root, or a network root + local destination (reusing `ConsoleImport`'s existing `romSource`/`localRomRoot` network-import plumbing); `emulator:scanLibrary` lists the root's immediate subfolders and proposes a console match per folder; a review screen lets the user correct/assign each one; on confirm, builds a `{folder, consoleKey}` queue and renders `ConsoleImport` once per queue item (`initialConsole` + `initialRomDirs` + a progress `title`), advancing to the next item when the current one's `onClose` fires — reuses every existing detect/scan/review/import screen unchanged, no parallel import path |
...
### Switch update/DLC sync — whole-folder transfer (issue #441)

Eden has no headless install path for update/DLC `.nsp` files (GUI-only "File > Install Files to NAND", requiring the user's own prod.keys), and its installed content lives in one shared content-addressed NAND store (`nand/system/Contents/registered`), not a per-title folder — so it can't be synced like a save/state, and individual update files can't be matched reliably by title-ID convention either (naming varies too much in practice). Since Switch games live one-per-folder, the whole folder is instead treated as the sync unit: `emusync push`/`pull` — the same top-level commands every console already uses — tar up the base ROM's entire containing folder (base ROM + any update/DLC files sitting alongside it) and transfer that as one archive, reusing the exact folder-tar machinery already built for Wii/PS2 folder saves (`memcard_bytes`/`_write_memcard` in `server/sync_client.py`). No separate command group, managed folder, or per-file matching logic — this is not a Switch-specific feature bolted on, just the existing ROM push/pull made folder-aware for one console.

- **`rom_transfers`/`rom_pull_requests`** both carry a `kind` column (`'rom'` default, or `'rom-folder'`) — migrations 18/19. `destination_path` keeps its existing meaning exactly (the ROM's own eventual path, chosen via the same target/local-folder prompt as any other push/pull) — only how the bytes get placed there differs: written directly for `'rom'`, extracted as a tar into `dirname(destination_path)` for `'rom-folder'`.
- **`cli/transfer.py`**: `push_rom`/`pull_rom` pack `dirname(rom_path)` via `memcard_bytes()` into a temp tar when the game's console is `"Switch"`, tagging the transfer/pull-request `kind="rom-folder"`; `_receive_transfer` extracts via `_write_memcard(Path(dirname(destination_path)), tar_bytes)` instead of writing one file, and skips the RetroArch-content-folder reorganize heuristic (the folder's already organized); `_handle_pull_request` mirrors the same tar-on-fulfill for the source side. `_run_transfer_daemon`'s two dispatch points just thread `kind` through — no separate function per kind.
- **API**: `POST /games/{slug}/rom-transfer` accepts an optional `X-Transfer-Kind` header (default `"rom"`); `POST /games/{slug}/rom-pull-request` accepts an optional `kind` body field (default `"rom"`); both persisted and surfaced in their `pending` listings and SSE payloads (`rom_transfer_queued`/`rom_pull_requested`).
- **GUI push**: `gui/electron/sync/rom.ts`'s `rom:push` handler (the existing per-game Push button) packs the folder via `spawnSync("tar", ["-cf", "-", "-C", dirname(gd.rom_path), "."])` — mirroring `memcard.ts`'s existing PS2 folder-memcard tar approach — when the game's console is Switch, tagging `X-Transfer-Kind: rom-folder`. Receiving happens through the same embedded Python sync-daemon thread (`cli/server.py`) regardless of which UI (or device) initiated the transfer.
- **Import wizard**: no per-file detection or storage at all — `cli/console.py`'s `_count_switch_sibling_files(rom_path)` (mirrored in `gui/electron/emulator/scan.ts`'s `countSwitchSiblingFiles()`) is a pure FYI count of other `.nsp`/`.xci` files next to the ROM, shown next to the save/state tags at import time; nothing is stored on `game_devices` for it, since the whole folder syncs as a unit at transfer time regardless of what's in it.
- **Import-time save path is always blank, never guessed** (`cli/console.py`'s `_switch_save_match()`, mirrored inline in `scan.ts`'s per-ROM mapping): every other console's import step guesses a save path from the ROM's filename/location (`<save_dir>/GameName/GameName.ext`), which is at worst unconfirmed until the emulator actually writes there. For Switch that guess is actively *wrong* — Eden's real save folder is a NAND path keyed by profile-ID + title-ID, unrelated to the ROM's name or folder — so it's left blank at import and learned after first play instead (`cli/run_switch.py`). `GameConfig.tsx`'s save-path-required validation is relaxed for `gameConsole === "Switch"` accordingly (an empty save path is the correct pre-first-play state, not a validation error).
- **GUI Settings tab** (`GameConfig.tsx`): for a Switch game (`gameConsole === "Switch"`, the game's console abbr fetched via `getGame`), the "States folder" input-group is swapped for a "Game folder" one in the exact same structure as the ROM file/save file rows above it (text field + icon buttons) — there's no separately-stored folder field, it's derived as `dirname(romPath)`. Buttons: open the folder in the OS file manager and add a file (e.g. a DLC `.nsp`) straight into it via a picker, backed by `gui/electron/gamefolder.ts`'s `gamefolder:reveal`/`gamefolder:addFile` IPC (`shell.openPath` / a plain file copy). Since Eden has no save-state feature at all (unlike every other emulator here, where states are just not-yet-used), `SaveHistory.tsx`'s `hideStates` prop (passed for Switch in `GameModal.tsx`) also skips the state-history fetch/display entirely rather than showing a perpetually-empty section.

### Switch mod pool — communal, name-keyed sync (issue #444)

Eden stores each installed mod for a title as its own subfolder under `load/<title-id-hex>/<mod-name>/` (e.g. `load/0100000011D90000/Luminescent Platinum 2.2/`), keyed by the same title ID as the NAND save folder (#443's `games.switch_title_id`). Unlike saves, mod content is large (1.4GB seen in production) and rarely changes once installed, so the sync model is deliberately different: a communal, purely-additive pool keyed by `(title_id, mod_name)`, synced by a name-only diff rather than content hashing or newest-wins.

- **`switch_mods` table** (schema v22): `title_id`, `mod_name`, `size`, `pushed_by`, `pushed_at`, PK `(title_id, mod_name)`. `server/store/switch_mods.py`'s `SwitchModMixin`: `add_switch_mod` is additive-only — a push whose `(title_id, mod_name)` already exists in the pool discards the incoming bytes and returns `False` rather than overwriting (two devices independently having the same mod is a routine no-op, not a conflict); `list_switch_mods(title_id)`; `get_switch_mod_path(title_id, mod_name)`. No generation history like saves — a mod's on-disk file just lives at `<data_dir>/blobs/switch_mods/<title_id>/<mod_name>.tar`, overwritten only on that first push. A device removing its own local mod copy never deletes anything from the pool or other devices — there is no delete propagation at all.
- **API** (`server/api/mods.py`): `GET /switch/{title_id}/mods` (list), `GET`/`POST /switch/{title_id}/mods/{mod_name}` (download/upload raw tar bytes, mirroring the save blob routes' streaming upload/`FileResponse` pattern).
- **`SyncClient`**: `list_switch_mods(title_id)`, `push_switch_mod(title_id, mod_name, folder_path)`, `pull_switch_mod(title_id, mod_name, dest_path)` — reuse `memcard_bytes`/`_write_memcard` verbatim (a mod folder packs exactly like a save folder), no new tar-handling code.
- **`cli/run_switch.py`**: `_SWITCH_LOAD_ROOTS` (dual native/EmuDeck paths, mirrors `_SWITCH_NAND_ROOTS`); `_local_switch_mods(title_id)` lists every local mod subfolder by name; `_switch_load_root_for(title_id)` picks the destination root for a newly-pulled mod (prefers a root that already has this title's folder, then any existing root, then the primary root as a last resort); `_sync_switch_mods(client, title_id)` is the actual sync — a **name-only** set diff between local and pool: local-only names get pushed, pool-only names get pulled, and a name present on both sides is never re-verified or re-transferred. Every step is wrapped so a failure (network, a single bad mod) never raises — mod sync must never block a game launch.
- **Trigger**: wired into `cli/run.py`'s existing Switch branch of `run_game`, called once per launch (before the emulator starts) using whichever title ID got resolved that launch (stored `games.switch_title_id`, falling back to the filename-tag parse) — same resolution path already used for save matching/seeding (#443), computed once and reused rather than re-derived per call site.
- **CLI**: `cli/mod.py`'s `mod` group — `emusync mod list <slug>` (local vs. pool table) and `emusync mod sync <slug>` (manual trigger, same name-only logic as the automatic one).
- **GUI**: `gui/electron/switchmods.ts` — `switchMods:listLocal`/`switchMods:reveal` read/open the local `load/<title-id>/` folder (main-process-only filesystem access, mirrors `gamefolder.ts`); `switchMods:sync` shells out to `emusync mod sync <slug>` as a one-shot child process (mirrors `game.ts`'s `game:launch` spawn pattern) rather than duplicating the Python sync/tar logic in TypeScript. `SwitchModsTab.tsx` (new "Mods" tab on `GameModal.tsx`, shown only when `isSwitch`) merges the local listing with `GET /switch/{title_id}/mods` into one table (local ✓ / pool ✓ / size per mod) with "Sync mods now" and "Open mods folder" buttons.

### Switch title ID discovery by name lookup (issue #448)

`switch_title_id` used to only ever get populated by the bracket-tag filename convention (CLI import wizard only — `cli/console.py`) or by post-launch save-folder discovery (#443) — untagged ROMs imported through the GUI stayed blind until played once, since the GUI import wizard never attempted either path. Reading the real ID out of the ROM's own NSP/XCI header would need a user-supplied Switch decryption keyset (out of scope — no such key material is bundled or required); instead this matches the game's *display name* against a public catalog.

- **`gui/electron/switchtitledb.ts`** (new): `switchTitleDb:lookup` IPC takes a game name, returns a title ID or null. Backing data is [blawar/titledb](https://github.com/blawar/titledb)'s `US.en.json` — an unauthenticated, static name↔title-ID catalog (no keys/NCA decryption involved). The raw file is large (~85MB — it bundles descriptions/box-art URLs this feature doesn't need, and no lighter published dataset exists for the project), so it's fetched once, distilled down to a `slug -> titleId` map (base titles only, ID ending `000`, same filter convention as `_SWITCH_TITLE_ID_RE`), and cached to `~/.emusync/switch_titledb.json` (refetched if missing or >30 days old) — only the very first lookup ever pays for the full download. **Exact normalized-name match only, no fuzzy scoring**: a wrong guess would seed a save into the wrong title's Eden folder (same reasoning as the Wii NAND-folder gotcha), so a miss just returns null and the caller falls through to the existing bracket-tag/post-launch paths unchanged.
- **Import wizard**: `console-import/postImport.ts`'s `backfillSwitchTitleIds()` is called from `useConsoleImport.ts`'s `_runImport` after every Switch import — best-effort, fire-and-forget like the other post-import steps (art prefetch, auto-push), persists a match via `applySwitchTitleId()` (below).
- **App-wide startup backfill**: `App.tsx`'s `backfillAllSwitchTitleIds()` runs once per app session, gated on `useGameList`'s first successful load (a `hasBackfilledRef` guard, no separate polling) — lists every game via `listGames()`, filters to `console === "Switch" && !switch_title_id`, and runs the same lookup for each. Catches games imported before this feature existed, or whose GUI-wizard lookup found no match at the time, without needing a manual per-game click.
- **Manual backfill**: `SwitchModsTab.tsx`'s empty-title-ID state has a "Look up title ID" button running the same lookup + persist.
- **`applySwitchTitleId(slug, titleId)`** (`console-import/postImport.ts`, shared by all three call sites above): persists the match via `setGameSwitchTitleId` (`api.ts`, mirrors `setGameSgdbId`'s "fetch name, PUT with the new field" shape since the route requires `name`), then best-effort calls `switchTitleDb:ensureSaveFolder`.
- **`switchTitleDb:ensureSaveFolder` IPC** (`switchtitledb.ts`) shells out to the new `emusync game ensure-switch-save-folder <slug>` CLI command (`cli/game.py`, reuses `cli/mod.py`'s `_title_id_for` for the same Switch-game/known-title-ID validation `mod list`/`mod sync` already do) — creates an empty `<profile>/<title-id>/` placeholder in every profile `cli/run_switch.py`'s `_switch_profile_dirs()` finds, skipping any that already exist. Pure destination directory, no save bytes written — deliberately excluded from `_find_local_switch_save`'s "has real data" check (#443), so it's inert until the game is actually played or a save is pulled/seeded via `emusync run`'s existing `_seed_switch_save`. Mirrors `switchmods.ts`'s "shell out to the CLI rather than duplicate the Python logic in TypeScript" pattern.
- **Manual GUI Pull with no local save yet (#456)**: `GameConfig.tsx`'s sync panel used to require a known `save_path` to render at all, so a device that had never played a Switch game had no way to pull a save already synced from elsewhere — it had to wait for its own first launch. `showSyncPanel` now also renders for Switch with a blank `save_path`; its Pull button calls `window.emusync.save.pullSwitchSeed(slug)` (`save.ts`'s `save:pullSwitchSeed` IPC) instead of the normal `save:pull`, which shells out to `emusync game pull-switch-save <slug>` (`cli/game.py`) — reuses `_seed_switch_save` (the same helper `emusync run` seeds with pre-launch) to pull the server's save into whatever local Eden profile folder exists, then persists the destination as this device's `save_path` immediately (unlike the pre-launch seed, there's no upcoming launch here to adopt it afterward). The renderer re-fetches the device config on success to pick up the new `save_path`.

...
- **Import wizard**: `console-import/postImport.ts`'s `backfillSwitchTitleIds()` is called from `useConsoleImport.ts`'s `_runImport` after every Switch import — best-effort, fire-and-forget like the other post-import steps (art prefetch, auto-push), persists a match via the new `setGameSwitchTitleId` (`api.ts`, mirrors `setGameSgdbId`'s "fetch name, PUT with the new field" shape since the route requires `name`).
- **Backfill for already-imported games**: `SwitchModsTab.tsx`'s empty-title-ID state gets a manual "Look up title ID" button running the same lookup + persist, for games imported before this existed or whose first lookup found no catalog match.

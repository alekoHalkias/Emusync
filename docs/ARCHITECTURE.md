...
- **GUI "Folder" tab** (`GameFolderTab.tsx`, `gui/electron/gamefolder.ts`'s `gamefolder:list`/`gamefolder:reveal`/`gamefolder:addFile` IPC): local-file counterpart to the sync — lists the game's folder contents, reveals it in the OS file manager (`shell.openPath`), and lets the user copy a file (e.g. a DLC `.nsp`) straight into it via a file picker, no manual OS navigation needed. Shown first in `GameModal.tsx`'s tab order for Switch games (`consoleKey === "switch"`), pushing the other tabs down one position. Since Eden has no save-state feature at all (unlike every other emulator here, where states are just not-yet-used), `SaveHistory.tsx`'s `hideStates` prop (passed for Switch) skips the state-history fetch/display entirely rather than showing a perpetually-empty section.

...
- **GUI**: `gui/electron/sync/rom.ts`'s `rom:push` handler (the existing per-game Push button) packs the folder via `spawnSync("tar", ["-cf", "-", "-C", dirname(gd.rom_path), "."])` — mirroring `memcard.ts`'s existing PS2 folder-memcard tar approach — when the game's console is Switch, tagging `X-Transfer-Kind: rom-folder`. No separate GUI surface; receiving happens through the same embedded Python sync-daemon thread (`cli/server.py`) regardless of which UI (or device) initiated the transfer.
